...
from Diplom_2.methods.conftest import authorize_user, create_order, generate_user_credentials, \
    get_all_orders, get_ingredients_hashes, get_orders, register_user


class TestGetOrders:

    @allure.title('Запрос заказов с авторизацией возвращает код 200')
    ...
    @allure.title('Запрос заказов с авторизацией возвращает код 401')
    ...
    @allure.title('Запрос заказов без авторизации возвращает код 200')
